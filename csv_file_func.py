...
def writelist_to_file(cont: list, rd: str = 'w'):
...
def displaylist(temp: list):  # Display list in file format.
    for i in temp:
        printer(i)


# Converts date to yyyy-mm-dd from string
def managedate(date: str) -> list:  # input date- mm/dd/yyyy
...
def modifysnolist(var: list) -> list:
    cnt = 0
    for i in var:
        cnt += 1
        i[_Sno_] = cnt
...
def updateSNO(file, cont: list, sno: int):
    tempstor = filelist(file)
...
def search_sno(file, sno: int, opr='1') -> int | None:
    tempstor = filelist(file)
    header = tempstor[0]
    flag = True
    if opr == "1":
...
def better_srch_name(file, name: str) -> int:
    tempstor = filelist(file)
    names = []
    for i in tempstor:
...
def search_empid(file, parameter_empid: int) -> int | None:
    tempstor = filelist(file)
    for i in tempstor[1::]:
...
def auto_search(file, inp: int | str) -> int:
    try:
        inp = int(inp)
...
def search_salary(file, parameter_salary: int, operator: str = '1'):
    tempstor = filelist(file)
    header = tempstor[0]
    flag = True
    flag1 = True
...
# Search with dates
def srch_date(file, inp_date: str, opr="1"):
    tempstor = filelist(file)
    header = tempstor[0]
...
# Search by division
def search_divion(file, parameter_division: str):
    tempstor = filelist(file)
    flag1 = True
    for row in tempstor:
...
# Sorts by date
def sort_date(file, desc=False) -> list:
    tempstor = filelist(file)

    def function(x: list) -> list:
